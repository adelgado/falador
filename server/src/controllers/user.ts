...
import { default as User, IAuthToken, UserModel } from '../models/User'
...
export let postLogin = (req: Request, res: Response, next: NextFunction) => {
	...
	passport.authenticate(
		'local',
		(err: Error, user: UserModel, info: LocalStrategyInfo) => {
			if (err) {
				return next(err)
			}
			if (!user) {
...
export let postReset = (req: Request, res: Response, next: NextFunction) => {
	...
	async.waterfall(
		[
			function resetPassword(done: (error: any, user: any) => void): void {
				User.findOne({ passwordResetToken: req.params.token })
					.where('passwordResetExpires')
					.gt(Date.now())
					.exec((err, user: any) => {
	...
	async.waterfall(
		[
			...
			function setRandomToken(
				token: IAuthToken,
				done: (err: WriteError, token?: IAuthToken, user?: any) => void
			): void {
